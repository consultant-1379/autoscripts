...
my %opts = (
	...
	'operation' => {
        ...
        'nictype' => {
	        type => "=s",
        	help => "pcnet|vmxnet2|vmxnet3|e1000",
	        required => 0,
        },
	'mac' => {
                type => "=s",
...
sub updatemac() {
	...
	if ( exists $vnic_device->backing->{network} ) {
	    $network = Vim::get_view(mo_ref => $vnic_device->backing->network, properties => ['name']);
	    #print "Network: " . $network->{'name'} . "\n\n";
	    $backing_info->{deviceName} = $network->{'name'};
	} else { 
	    # We need to give something for the deviceName
...
sub updatenictype() {
	...
	my $backing_info = $vnic_device->backing;
	...
	my ($task_ref,$msg);
        ...
        $vm_dev_spec = VirtualDeviceConfigSpec->new(device => $vnic_device,operation => $config_spec_operation);
