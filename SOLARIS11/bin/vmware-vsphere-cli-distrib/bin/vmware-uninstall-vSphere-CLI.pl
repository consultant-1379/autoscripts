...
sub acevm_included_player_newer {
  return 615831 gt int($gConfig->get('product.buildNumber', ''));
}
...
sub install_content_vicli_perl {
   ...
   if ( file_name_exist("/etc/redhat-release") && direct_command("cat /etc/redhat-release | grep \"Red\ Hat\ Enterprise.*5\.5\"") ) {
      my $msg = "This " . vmware_product_name() . " installer includes precompiled Perl modules for RHEL 5.5.\n"
                . "Answering yes will install the precompiled modules, and answering no will install from CPAN.\n"
                . "Do you want to install precompiled Perl modules for RHEL 5.5?\n";
      if (get_answer($msg, 'yesno', 'yes') eq 'yes') {
         $install_rhel55_local = 1;
      }
   }
   ...
   my @modules = (
    ...
    {'module' => 'Compress::Zlib',                'version' => '2.037',    'path' => 'Compress-Zlib-2.037'},
    {'module' => 'Compress::Raw::Zlib',           'version' => '2.037',    'path' => 'Compress-Raw-Zlib-2.037'},
    ...
    {'module' => 'IO::Compress::Base',            'version' => '2.037',    'path' => 'IO-Compress-Base-2.037'},
    {'module' => 'IO::Compress::Zlib::Constants', 'version' => '2.037',    'path' => 'IO-Compress-Zlib-2.037'},
    ...
    {'module' => 'LWP::Protocol::https',          'version' => '5.805',    'path' => 'LWP-Protocol-https-5.805'}
   );
...
sub install_content_vicli {
  ...
  db_add_answer('BUILD_NUMBER', "615831");
...
sub main {
   ...
   if (internal_basename($0) eq $cInstallerFileName) {
    ...
    if (vmware_product() ne 'api' && vmware_product() ne 'acevm' &&
        ...
        vmware_product() ne 'vicli' && vmware_product() ne 'vix-disklib') {
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '5.0.0') and
