...
sub update_drive{
	...
	foreach my $device (@$devices){
		...
		if($device->isa("VirtualCdrom")) {
			...
			$is_emulate = $backing->isa($backing_type);
			if (!$is_emulate)
			{
			...
			my $virtualdevice = VirtualCdrom->new(
					...
					key => $key,
					backing => $backing_type->new(deviceName => 'No Devices available'),
...
sub flip_mode()
{
        update_drive('VirtualCdromRemoteAtapiBackingInfo');
        update_drive('VirtualCdromAtapiBackingInfo');
	Util::disconnect();
        exit 0;
}
