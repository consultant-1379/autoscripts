...
ACTUAL_DHCP_SERVER='ieatrcxb4936-1'
ACTUAL_DHCP_SERVER_IP='10.45.231.231'
ACTUAL_DHCP_SERVER_ROOT_PASS='shroot12'
...
function simdep_call ()
{
		...
		message "INFO:-$functionName: end overriding default simdep config file \n" INFO
                ...
                $SSH -qTn $netsim_server "$MOUNTPOINT/bin/simdep_caller.sh ${SIMULATIONDROP}"
                if [[ $? -ne 0 ]]
                then
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        remove_dhcp_client_remote $OMSERVS_HOSTNAME
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        remove_dhcp_client_remote $MWS_HOSTNAME
        clear_lock /tmp/MWS.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
set prompt ".*(%|#|\\$|>):? $"
set timeout 300
spawn $SSH -qtn -l $SSHUSER $SERVER "ls $SCRIPTMOUNT/CLOUD_xlaxain/bin/" 2>/dev/null
...
#####
	...
	if [[ "$NETSIM_SERVERS" != "" ]] && [[ "$INITIAL_INSTALL_NETSIM" == "yes" ]] && [[ "$INITIAL_INSTALL_NETSIM_PART2" != "no" ]]
        then
                ...
                (
                (
                        parallel_requirements "$REQUIREMENT_NETSIM_ROLLOUT_PART1 $REQUIREMENT_OMSAS_POST_STEPS"
                        netsim_rollout_part2
                ) > $LOG_FILE 2>&1;parallel_finish
                ) & set_parallel_variables
        fi
...
EOF
	...
	mount_scripts_directory $EBAS_HOSTNAME
	$SSH -qt $EBAS_HOSTNAME "$MOUNTPOINT/bin/sol11_copy_p12_to_ebas.sh  -c '$CONFIG' -m $MOUNTPOINT"
	if [[ $? -ne 0 ]]
	then
