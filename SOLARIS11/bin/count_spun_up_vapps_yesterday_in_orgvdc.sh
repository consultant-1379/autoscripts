...
check_args
OUTPUT=`$MOUNTPOINT/bin/vCloudFunctions_php.sh --username=script --vcloudphphostname=$VCLOUD_PHP_HOSTNAME -f count_spun_up_vapps_yesterday_in_orgvdc --orgvdcname="$ORGVDC" 2>&1`
if [[ $? -ne 0 ]]
then
	echo "error"
	echo "$OUTPUT"
