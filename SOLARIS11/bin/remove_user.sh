...
if pgrep ns-slapd > /dev/null; then
        DEL_USER_SCRIPT=/ericsson/sdee/bin/del_user.sh
else
        DEL_USER_SCRIPT=/ericsson/opendj/bin/del_user.sh
fi
...
spawn ${DEL_USER_SCRIPT} -d $LDAPDOMAIN
while {true} {
	expect	{
		-re "ERROR:(.*)$" {
		...
		"OK: Account deleted:"
		{
			send_user "${SPECIAL}User $USERNAME was removed successfully"
			exit 0
		}
		-re "com]: OK" {
                        send_user "${SPECIAL}User $USERNAME was removed successfully"
		...
		timeout {
			send_user "ERROR: Timeout when removing user\n"
			exit 3
		}
	}
