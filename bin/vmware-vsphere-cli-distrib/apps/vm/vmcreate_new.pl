...
#   foreach (@vms) {
	...
	my $memoryMB="";
	my $numCPUs="";
	my $bootOptions = VirtualMachineBootOptions->new(bootDelay => 0);
	...
	foreach $line (<F>) {
		...
		if ("$values[0]" eq "new_hard_disk")
		{
			...
			my $disk_vm_dev_conf_spec = create_virtual_disk(ds_path => $ds_path, disksize => $disksizeKB, provisionType => $values[2], iolimit => $iolimit);
		...
		elsif ("$values[0]" eq "scsi_controller")
		{
			...
			my $vsp_back = VirtualSerialPortURIBackingInfo->new(proxyURI => "telnet://" . $values[1] . ":13370" , serviceURI => "vSPC.py", direction => "server" );
		...
		elsif ("$values[0]" eq "bootdelay")
		{
			$bootOptions = VirtualMachineBootOptions->new(bootDelay => $values[1]);
		}
		}
	}
	close (F);
