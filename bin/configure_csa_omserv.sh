...
spawn ssh -t -o UserKnownHostsFile=/dev/null -o CheckHostIP=no -o StrictHostKeyChecking=no root@$OMSAS_HOSTNAME "/opt/ericsson/secinst/bin/config.sh -R /var/tmp/secinst_`hostname`_config.zip -v -v -v"
while {"1" == "1"} {
expect {
        "assword:" {send "shroot\r"}
        eof {
