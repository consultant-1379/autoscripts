...
function assign_netsim_addresses()
{
	...
	local IPV6="$4"
        ...
        get_lock $ADDRESSES_LOCK local na 18000 yes
...
function uncompress_sim ()
{
	...
	local SIM_FILENAME="$2"
        ...
        get_lock $UNCOMPRESS_LOCK local na 18000 yes
...
function manage_mcs_internal ()
{
	...
	get_lock $MC_LOCK local na 18000 yes
...
function cpu_usage_logging ()
{
	mount_scripts_directory $ADM1_HOSTNAME
	$SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/cpu_usage_logging.sh"
}
function generate_p12_omsas ()
{
	...
	exec_configuration_script "bin/generate_p12_omsas.sh"
	local EXIT_CODE=$?

	if [[ $EXIT_CODE -ne 0 ]]
	then
		message "ERROR: Something went wrong configuring generate_p12_omsas, please check output above" ERROR
		exit 1
	fi
}
...
function copy_ms_certs_omsas ()
{
	...
	#wait_oss_online_adm1
        mount_scripts_directory $OMSAS_HOSTNAME
	# loop through the configuration script and retry if there are any issues
	exec_configuration_script "bin/copy_ms_certs_omsas.sh"
	local EXIT_CODE=$?

	if [[ $EXIT_CODE -ne 0 ]]
	then
		message "ERROR: Something went wrong configuring copy_ms_certs_omsas, please check output above\n" ERROR
		exit 1
	fi
}
...
nm_jks_exist=`$SSH  -qt $ADM1_HOSTNAME ls /opt/ericsson/nms_netconf/security/nm_netconf.jks | wc -l`
if [[ $nm_jks_exist -eq 0 ]]
        then
                      message  "ERROR: nm_netconf.jks does not exist\n" ERROR
                     exit 1
        fi
}
...
function uas_post_steps_internal ()
{
	...
	$SSH -qt $SERVER "$MOUNTPOINT/bin/uas_post_steps.sh -m $MOUNTPOINT -c '$CONFIG'"
...
function copy_p12_to_server_prefix ()
{
	local PREFIX=$1
        local X_HOSTNAME=`eval echo \\$${PREFIX}_HOSTNAME`
	mount_scripts_directory $X_HOSTNAME
	$SSH -qt $X_HOSTNAME "$MOUNTPOINT/bin/copy_p12_to_ebas.sh  -c '$CONFIG' -m $MOUNTPOINT"
}
...
function setup_adm1_ldap_client ()
{
	#wait_oss_online_adm1
	mount_scripts_directory $ADM1_HOSTNAME
	$SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/setup_adm1_ldap_client.sh  -c '$CONFIG' -m $MOUNTPOINT"
	if [[ $? -ne 0 ]]
	then
...
function remake_cluster()
{
	...
	# Unmount stale /export/scripts/ mounts after cutover
        setup_passwordless_ssh $BACKUP_IP root shroot
        $SSH -qt $BACKUP_IP "umount -f /export/scripts/;sleep 5;umount -f /export/scripts/"

        setup_passwordless_ssh $ADM2_BACKUP_IP root shroot
	...
	while [ $i -lt 4 ]; do
		...
		if [ $EXIT_CODE -eq 99 ] || [ $EXIT_CODE -eq 98 ] || [ $EXIT_CODE -eq 97 ]; then
			if [ $EXIT_CODE -eq 99 ]; then
				...
				ERROR="Timeout issue, did not detect string \"Preparing system for Solaris install\""
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $ADM1_HOSTNAME
...
while {"1" == "1"} {
        expect {
        "install_veritas_patches successfully executed"
        {
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $OMSAS_HOSTNAME
        clear_lock /tmp/OMSAS.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
while {"1" == "1"} {
	...
	"Select a Language" {
        ...
        "SUN_INST_PATCH.sh: Cleaning up and exiting"
        {
                send_user "\nDetected patch installation completed\n"
		while {"1" == "1"} {
			expect {
				"values correct" {
                                        send "y\r"
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client $OMSERVS_HOSTNAME
        clear_lock /tmp/OMSERVS.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $OMSERVM_HOSTNAME
        clear_lock /tmp/OMSERVM.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $UAS1_HOSTNAME
        clear_lock /tmp/UAS1.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $PEER1_HOSTNAME
        clear_lock /tmp/PEER1.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $EBAS_HOSTNAME
        clear_lock /tmp/EBAS.lock remote $ACTUAL_DHCP_SERVER_IP
fi
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $ADM2_HOSTNAME
        clear_lock /tmp/$PREFIX.lock remote $ACTUAL_DHCP_SERVER_IP
fi
if [[ $EXIT_CODE -ne 0 ]]
then
...
if [[ "$BEHIND_GATEWAY" == "yes" ]]
then
        #remove_dhcp_client_remote $NEDSS_HOSTNAME
        clear_lock /tmp/NEDSS.lock remote $ACTUAL_DHCP_SERVER_IP
fi
if [[ $EXIT_CODE -ne 0 ]]
then
	message "ERROR: Installation did not complete successfully\n" ERROR
...
function remove_dhcp_client_remote ()
{
	...
	ssh $DHCP_SERVER_IP "/ericsson/jumpstart/bin/manage_dhcp_clients.bsh -a remove -c $SERVER -N"
...
function add_dhcp_client_remote()
{
	...
	do
		...
		then
			...
			MANAGE_DHCP_SCRIPT="/ericsson/jumpstart/bin/manage_dhcp_clients.bsh"
		fi

		ssh $DHCP_SERVER_IP "$MANAGE_DHCP_SCRIPT -a add -f $CLIENT_CONFIG_FILE -N"
	...
	if [[ "$BEHIND_GATEWAY" == "yes" ]]
        then
		clear_lock /tmp/${CLIENT_TYPE}.lock remote $ACTUAL_DHCP_SERVER_IP 18000 yes
	fi
	exit 1
...
function jsbsh ()
{
	...
	INPUT_IP=$2
	INPUT_MAC=$3
	JS_LOCK="/tmp/$PARENT_BASHPID/locks/js.lock"
	get_lock $JS_LOCK local na 18000 yes
	message "INFO: Running /root/js.bsh to enable dhcp through the gateway\n" INFO
	bash /root/js.bsh $INPUT_HOSTNAME $INPUT_IP $INPUT_MAC
	clear_lock $JS_LOCK local na
}
...
CLIENT_INSTALL_PARAMS@- install inst_type=oss_client config=appserv
EOF
`
...
CLIENT_INSTALL_PARAMS@- install label_disks inst_type=oss_client config=peer rmirr
EOF
`
...
CLIENT_INSTALL_PARAMS@- install inst_type=oss_client config=ebas
EOF
`
...
CLIENT_INSTALL_PARAMS@- install inst_type=cominf config=infra_omsas nox
EOF
`
...
CLIENT_INSTALL_PARAMS@- install inst_type=cominf config=om_serv_master nox
EOF
`
	echo "$JUMP_OUTPUT" > $CONFIG_FILE
        message "DHCP Client file written for this server at $CONFIG_FILE\n" INFO
...
CLIENT_INSTALL_PARAMS@- install inst_type=cominf config=om_serv_slave nox
EOF
`
        echo "$JUMP_OUTPUT" > $CONFIG_FILE
        message "DHCP Client file written for this server at $CONFIG_FILE\n" INFO
...
function create_config_files_internal()
{
	...
	local X_Numradio=`eval echo \\$${PREFIXWADM}Numradio`
        ...
        local X_Nummrsv=`eval echo \\$${PREFIXWADM}Nummrsv`
        local X_Numrouter_6672=`eval echo \\$${PREFIXWADM}Numrouter_6672`
	local X_Numipworks=`eval echo \\$${PREFIXWADM}Numipworks`
	...
	then
		...
		then
			message "INFO: Obtaining veritas license from 159.107.220.94:/export/veritas/LICENCES.txt\n" INFO
			mkdir /tmp/veritas >/dev/null 2>&1
			mount -o vers=3 159.107.220.94:/export/veritas /tmp/veritas >/dev/null 2>&1
		fi
...
CLIENT_INSTALL_PARAMS@- install label_disks inst_type=ossrc config=system $X_EXTRA_BOOTARGS pre_ini=$MWS_IP:$PRE_INIRATOR_DIR/
EOF
`
...
Numradio=$X_Numradio
; number of MRSV nodes the OSSRC will support
Nummrsv=$X_Nummrsv
; number of ROUTER_6672 nodes the OSSRC will support
Numrouter_6672=$X_Numrouter_6672
; number of IPWORKS nodes the OSSRC will support
Numipworks=$X_Numipworks
; Installation type
INS_TYPE=ii
...
function setup_ssh_masterservice()
{
	...
	local SSH_MASTERSERVICE_LOCK="/tmp/$PARENT_BASHPID/locks/setup_ssh_masterservice.lock"
        get_lock $SSH_MASTERSERVICE_LOCK local na 18000 yes
        $SSH -qt $ADM1_HOSTNAME  "su - comnfadm -c \"mkdir -p /home/comnfadm/.ssh\""
        $SSH -qt $FROM_SERVER "$MOUNTPOINT/bin/setup_ssh_masterservice.sh -m $MOUNTPOINT -c '$CONFIG'"
...
function configure_smrs_master_service ()
{
	...
	mount_scripts_directory $ADM1_HOSTNAME
	$SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/configure_smrs_master_service.sh  -c '$CONFIG' -m $MOUNTPOINT" 2>/dev/null
	if [[ $? -ne 0 ]]
        then
...
function configure_smrs_add_nedss_prefix()
{
	local PREFIX="$1"
        ...
        mount_scripts_directory $ADM1_HOSTNAME
        $SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/configure_smrs_add_nedss.sh  -c '$CONFIG' -m $MOUNTPOINT -p $PREFIX" 2>/dev/null
	if [[ $? -ne 0 ]]
        then
...
function configure_smrs_add_slave_service_prefix ()
{
	...
	mount_scripts_directory $ADM1_HOSTNAME
	$SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/configure_smrs_add_slave_service.sh  -c '$CONFIG' -m $MOUNTPOINT -p $PREFIX -i "$IPV6"" 2>/dev/null
        if [[ $? -ne 0 ]]
        then
...
function setup_ntp_client()
{
	...
	mount_scripts_directory $SERVER_IN
        $SSH -qTn $SERVER_IN "$MOUNTPOINT/bin/setup_ntp_client.sh  -c '$CONFIG' -m $MOUNTPOINT"
}
...
function configure_csa_omsas()
{
	if [[ "$CONFIGURE_CSA" != "NO" ]]
		...
		mount_scripts_directory $OMSAS_HOSTNAME
		# loop through the configuration script and retry if there are any issues
		exec_configuration_script "bin/configure_csa_omsas.sh" "2>/dev/null"
		local EXIT_CODE=$?
		if [[ $EXIT_CODE -ne 0 ]]
		then
...
function configure_csa_omservm()
{
	...
	then
		...
		$SSH -qt $OMSERVM_HOSTNAME "$MOUNTPOINT/bin/configure_csa_omserv.sh  -c '$CONFIG' -m $MOUNTPOINT -a $ADMIN_SERVER_PRESENT" 2>/dev/null
	fi
	if [[ $? -ne 0 ]]
        then
...
function configure_csa_omservs()
{
	...
	then
		...
		$SSH -qt $OMSERVS_HOSTNAME "$MOUNTPOINT/bin/configure_csa_omserv.sh  -c '$CONFIG' -m $MOUNTPOINT -a $ADMIN_SERVER_PRESENT" 2>/dev/null
	fi
	if [[ $? -ne 0 ]]
        then
...
function setup_resolver()
{
	local SERVER_IN=$1
	mount_scripts_directory $SERVER_IN
	$SSH -qt $SERVER_IN "$MOUNTPOINT/bin/setup_resolver.sh  -c '$CONFIG' -m $MOUNTPOINT" 2>/dev/null
}
...
function setup_remote_passwordless_ssh()
{
	...
	mount_scripts_directory $FROM_SERVER
	$SSH -qt $FROM_SERVER "$MOUNTPOINT/bin/setup_remote_passwordless_ssh.sh -m $MOUNTPOINT -t $TO_SERVER"
	local EXIT_CODE=$?
        if [[ $EXIT_CODE -ne 0 ]]
        then
...
function setup_adm1_ssh_keys ()
{
	...
	$SSH -qt $ADM1_HOSTNAME "$MOUNTPOINT/bin/setup_adm1_ssh_keys.sh"
}
...
function all_eth_post_steps ()
{
	...
	update_root_password $ADM1_HOSTNAME "shroot"
	echo_line
...
function rollout_config ()
{
	...
	if [[ "$ADM1_HOSTNAME" != "" ]] && [[ "$POST_INSTALL_ADM1" == "yes" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
                ...
                (
                (
                        parallel_requirements "$REQUIREMENT_ADM1_PART2 $REQUIREMENT_OMSERVM_POST_STEPS $REQUIREMENT_OMSAS_POST_STEPS"
			wait_oss_online_adm1
			wait_until_services_started_adm1
			wait_until_services_started_omservm
                        setup_adm1_ldap_client
	...
	if [[ "$ADM1_HOSTNAME" != "" ]] && [[ "$POST_INSTALL_ADM1" == "yes" ]] && [[ "$ADM1_SECURITY" == "yes" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
                ...
                (
                        ...
                        then
                                parallel_requirements "$REQUIREMENT_OMSAS_POST_STEPS $REQUIREMENT_OMSERVM_POST_STEPS $REQUIREMENT_OMSERVS_POST_STEPS"
                                wait_oss_online_adm1
                                wait_until_services_started_adm1
                                wait_until_services_started_omsas
                                wait_until_services_started_omservm
                                ...
                                parallel_requirements "$REQUIREMENT_OMSAS_POST_STEPS $REQUIREMENT_OMSERVM_POST_STEPS"
                                wait_oss_online_adm1
                                wait_until_services_started_adm1
                                wait_until_services_started_omsas
                                wait_until_services_started_omservm
	...
	if [[ "$UAS1_HOSTNAME" != "" ]] && [[ "$POST_INSTALL_UAS1" == "yes" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
                ...
                (
                (
			parallel_requirements "$REQUIREMENT_ADM1_INITIAL_JUMP_COMPLETE $REQUIREMENT_ADM1_PART1 $REQUIREMENT_ADM1_PART2 $REQUIREMENT_UAS1_PART2 $REQUIREMENT_SETUP_ADM1_LDAP_CLIENT"
			if [[ "$UAS1_SERVER_TYPE" == "blade" ]]
			then
				:	
			else
	...
	if [[ "$PEER1_HOSTNAME" != "" ]] && [[ "$POST_INSTALL_PEER1" == "yes" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
                ...
                (
                (
			parallel_requirements "$REQUIREMENT_PEER1_PART2 $REQUIREMENT_SETUP_ADM1_LDAP_CLIENT $REQUIREMENT_ADM1_PART2 $REQUIREMENT_OMSERVM_POST_STEPS $REQUIREMENT_OMSAS_POST_STEPS"
			if [[ "$PEER1_SERVER_TYPE" == "blade" ]]
			then
				:
			else
	...
	if [[ "$EBAS_HOSTNAME" != "" ]] && [[ "$POST_INSTALL_EBAS" == "yes" ]]
		...
		LOG_FILE=$RUNNING_LOG_DIR/$PARALLEL_ID.log
                ...
                (
                (
                        parallel_requirements "$REQUIREMENT_EBAS_PART2 $REQUIREMENT_SETUP_ADM1_LDAP_CLIENT"
			if [[ "$EBAS_SERVER_TYPE" == "blade" ]]
			then
				:
			else
...
function plumb_storage_nic_prefix () {
        ...
        $SSH -qt $SERVER "$MOUNTPOINT/bin/plumb_storage_nic.sh -m $MOUNTPOINT -c '$CONFIG' -p $SERVER_PREFIX"
}
...
function get_vapp_lock ()
{
	...
	then
		...
		get_lock $VAPP_LOCK local na 18000 yes > /dev/null 2>&1
	fi
}
...
function fetch_ior_files ()
{
	...
	# loop through the configuration script and retry if there are any issues
	exec_configuration_script "bin/fetch_ior_files.sh"
	local EXIT_CODE=$?
...
function omsas_config_aiws ()
{
	...
	# loop through the configuration script and retry if there are any issues
	exec_configuration_script "bin/omsas_config_aiws.sh"
	local EXIT_CODE=$?

	clear_lock $MC_LOCK local na
	if [[ $EXIT_CODE -ne 0 ]]
	then
	       message "ERROR: Something went wrong configuring omsas_config_aiws, please check output above\n" ERROR
	       exit 1
	fi
}
...
EOF
	...
	mount_scripts_directory $EBAS_HOSTNAME
	$SSH -qt $EBAS_HOSTNAME "$MOUNTPOINT/bin/copy_p12_to_ebas.sh  -c '$CONFIG' -m $MOUNTPOINT"
	if [[ $? -ne 0 ]]
	then
