...
check_args
ssh -l root -qTn -o StrictHostKeyChecking=no -o PasswordAuthentication=no -o NumberOfPasswordPrompts=0 $VCLI "source /opt/vmware/vma/bin/vifptarget --set $VCENTER;$MOUNTPOINT/bin/vmware-vsphere-cli-distrib/apps/vm/vms_list_powered_on.pl --cluster '$CLUSTER'"
if [[ $? -ne 0 ]]
then
